...
do
   ...
   do 
       ...
       grep App       dc-300-$dc.output |grep socket |awk '{print $4}'  > App-time

       paste App parallel 0 1 2 3 App-time> temp
       sort -k 7 -r -n temp |head -n 9 |tail -n 8 > Ap0123-$dc
       cp Ap0123-$dc temp
       sort -k 2 -r -n Ap0123-$dc | awk '{a+=$1;p+=$2;b+=$3;c+=$4;d+=$5;e+=$6;} END {print '$dc' "\t" '$dc'/16*100 "%\t" a/NR "	" p/NR "	" b/NR "	" c/NR "	" d/NR "	" e/NR }' 
       rm Ap0123-$dc
       rm 0 1 2 3 App parallel App-time
   done
done
